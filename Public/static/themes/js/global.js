...
function autoheader() {
  if (
    ...
    if (
      $(window).scrollTop() >
      $(".banner_frame").height() - $(".header").height()
    ) {
...
$(function () {
  ...
  //search
  $(".header .sc,.header2 .sc,.body_r .navg .sc,.nav_mobile .sc").click(
    function () {
      showlayer("#search");
      setTimeout(function () {
        $(".search_box .kw").focus();
  ...
  $("body").mousewheel(function (event, delta, deltaX, deltaY) {
    ...
    if (
      deltaY < 0 &&
      $(".body_home").length == 0 &&
      $(".body").outerHeight() + $(".footer").outerHeight() > $(window).height()
    ) {
      $(".header").addClass("lit");
    }
  });
