...
(function ($) {
  $.fn.mScroll = function (options) {
    ...
    this.each(function () {
      ...
      //创建c镜像，赋值c
      if (opts.cycle == 1 && size > getMinsize()) {
        s.append(s.html());
      }
      c = s.children();
      ...
      if (opts.scrollnav) {
        wn = parseInt(
          n.outerWidth() +
            parseInt(n.css("margin-left")) +
            parseInt(n.css("margin-right"))
        );
        hn = parseInt(
          n.outerHeight() +
      ...
      function setH() {
        ...
        c.css("width", "");
        cm =
          parseInt(c.last().css("margin-left")) +
          parseInt(c.last().css("margin-right"));
        if (cm == 0) {
        ...
        function autonav() {
          if (opts.nav_direction == "h") {
            if (np.width() == wn) {
              ...
              if (idx == 0) {
                np.stop().animate({ scrollLeft: 0 }, opts.navspeed);
              }
              if (idx == size - 1) {
                np.stop().animate({ scrollLeft: size * wn }, opts.navspeed);
              }
              if (np.width() % wn != 0) {
                console.log("宽度设置错误");
              }
          ...
          if (opts.nav_direction == "v") {
            ...
            if (idx == 0) {
              np.stop().animate({ scrollTop: 0 }, opts.navspeed);
            }
            if (idx == size - 1) {
              np.stop().animate({ scrollTop: size * hn }, opts.navspeed);
            }
          }
        }
        //导航点击
