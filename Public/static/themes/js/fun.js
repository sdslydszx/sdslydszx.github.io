...
function split_page(e, btn, num) {
  ...
  $(btn).click(function () {
    $(e)
      .children()
      .slice(0, parseInt($(e).attr("page") + num))
      .show();
  });
}
  ...
  function run(type) {
    ...
    if (type == 1) {
      ...
      if ($(frame).scrollLeft() > w2) {
        $(frame)
          .stop()
          .animate({ scrollLeft: w2 }, speen, "jswing", function () {
            sw = 1;
          });
      }
      fn(idx);
    ...
    if (type == 2) {
      ...
      if ($(frame).scrollLeft() >= w2 - w4) {
        ...
        $(frame)
          .stop()
          .animate({ scrollLeft: w2 - w4 }, speen, "jswing", function () {
            sw = 1;
          });
      }
    }
...
function mIscroll2(frame, child, prev, next, speen) {
  ...
  $(prev).click(function () {
    ...
    while (1) {
      ...
      w =
        w +
        $(child).eq(idx).outerWidth() +
        parseInt($(child).eq(idx).css("margin-left")) +
        parseInt($(child).eq(idx).css("margin-right"));
      if (w > $(frame).width()) {
        $(child)
          .slice(0, idx)
          .each(function (index, element) {
...
function fullFill(e, w, h, ww, hh) {
  if (w / h > ww / hh) {
    $(e).css({
      height: hh,
      width: (hh / h) * w,
      marginLeft: ((hh / h) * w - ww) * -0.5,
    });
  } else {
    $(e).css({
      width: ww,
      height: (ww / w) * h,
      marginTop: ((ww / w) * h - hh) * -0.5,
    });
  }
}
...
function movebox(e, c) {
  ...
  $("body").mousemove(function (event) {
    if (sw_m == 1) {
      ...
      if (ml > ww - $(e).outerWidth()) {
        ml = ww - $(e).outerWidth();
      }
      if (mt < 0) {
        mt = 0;
      }
      if (mt > wh - $(e).outerHeight()) {
        mt = wh - $(e).outerHeight();
      }
